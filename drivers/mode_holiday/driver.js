...
module.exports.capabilities.onoff.get = function(device_data, callback) {
	...
	if (virtualholiday instanceof Error) 
		return callback(virtualholiday);
...
module.exports.capabilities.onoff.set = function(device_data, onoff, callback) {
	...
	if (onoff) {
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_holiday_on', tokens, state, device_data, function(err, result) {
				if (err) 
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_holiday_off', tokens, state, device_data, function(err, result) {
				if (err) 
					return console.error(err);
				}
			);
...
Homey
	.manager('flow')
	.on('condition.virtual_holiday', function(callback, args) {
...
Homey
	.manager('flow')
	.on('action.virtual_holiday_action_on', function(callback, args) {
		var virtualholiday = getholiday(args.device.id);
		if (virtualholiday instanceof Error) 
			return callback(virtualholiday);
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_holiday_on', tokens, state, args.device, function(err, result) {
				if (err) 
					return console.error(err);
				}
			);
...
Homey
	.manager('flow')
	.on('action.virtual_holiday_action_off', function(callback, args) {
		var virtualholiday = getholiday(args.device.id);
		if (virtualholiday instanceof Error) 
			return callback(virtualholiday);
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_holiday_off', tokens, state, args.device, function(err, result) {
				if (err) 
					return console.error(err);
				}
			);
