...
module.exports.capabilities.onoff.get = function(device_data, callback) {
	...
	if (virtualquiet instanceof Error) 
		return callback(virtualquiet);
...
module.exports.capabilities.onoff.set = function(device_data, onoff, callback) {
	...
	if (onoff) {
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_quiet_on', tokens, state, device_data, function(err, result) {
				if (err) 
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_quiet_off', tokens, state, device_data, function(err, result) {
				if (err) 
					return console.error(err);
				}
			);
...
Homey
	.manager('flow')
	.on('condition.virtual_quiet', function(callback, args) {
...
Homey
	.manager('flow')
	.on('action.virtual_quiet_action_on', function(callback, args) {
		var virtualquiet = getquiet(args.device.id);
		if (virtualquiet instanceof Error) 
			return callback(virtualquiet);
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_quiet_on', tokens, state, args.device, function(err, result) {
				if (err) 
					return console.error(err);
				}
			);
...
Homey
	.manager('flow')
	.on('action.virtual_quiet_action_off', function(callback, args) {
		var virtualquiet = getquiet(args.device.id);
		if (virtualquiet instanceof Error) 
			return callback(virtualquiet);
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_quiet_off', tokens, state, args.device, function(err, result) {
				if (err) 
					return console.error(err);
				}
			);
