...
module.exports.capabilities.onoff.get = function(device_data, callback) {
	...
	if (virtualblind instanceof Error) 
		return callback(virtualblind);
...
module.exports.capabilities.onoff.set = function(device_data, onoff, callback) {
	...
	if (onoff) {
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_blind_on', tokens, state, device_data, function(err, result) {
				if (err) 
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_blind_off', tokens, state, device_data, function(err, result) {
				if (err) 
					return console.error(err);
				}
			);
...
Homey
	.manager('flow')
	.on('condition.virtual_blind', function(callback, args) {
...
Homey
	.manager('flow')
	.on('action.virtual_blind_action_on', function(callback, args) {
		var virtualblind = getblind(args.device.id);
		if (virtualblind instanceof Error) 
			return callback(virtualblind);
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_blind_on', tokens, state, args.device, function(err, result) {
				if (err) 
					return console.error(err);
				}
			);
...
Homey
	.manager('flow')
	.on('action.virtual_blind_action_off', function(callback, args) {
		var virtualblind = getblind(args.device.id);
		if (virtualblind instanceof Error) 
			return callback(virtualblind);
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_blind_off', tokens, state, args.device, function(err, result) {
				if (err) 
					return console.error(err);
				}
			);
