...
module.exports.capabilities.onoff.get = function(device_data, callback) {
	...
	if (virtualsecurity instanceof Error) 
		return callback(virtualsecurity);
...
module.exports.capabilities.onoff.set = function(device_data, onoff, callback) {
	...
	if (onoff) {
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_security_on', tokens, state, device_data, function(err, result) {
				if (err) 
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_security_off', tokens, state, device_data, function(err, result) {
				if (err) 
					return console.error(err);
				}
			);
...
Homey
	.manager('flow')
	.on('condition.virtual_security', function(callback, args) {
...
Homey
	.manager('flow')
	.on('action.virtual_security_action_on', function(callback, args) {
		var virtualsecurity = getsecurity(args.device.id);
		if (virtualsecurity instanceof Error) 
			return callback(virtualsecurity);
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_security_on', tokens, state, args.device, function(err, result) {
				if (err) 
					return console.error(err);
				}
			);
...
Homey
	.manager('flow')
	.on('action.virtual_security_action_off', function(callback, args) {
		var virtualsecurity = getsecurity(args.device.id);
		if (virtualsecurity instanceof Error) 
			return callback(virtualsecurity);
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_security_off', tokens, state, args.device, function(err, result) {
				if (err) 
					return console.error(err);
				}
			);
