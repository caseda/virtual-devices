...
module.exports.capabilities.onoff.get = function(device_data, callback) {
	...
	if (virtualparty instanceof Error) 
		return callback(virtualparty);
...
module.exports.capabilities.onoff.set = function(device_data, onoff, callback) {
	...
	if (onoff) {
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_party_on', tokens, state, device_data, function(err, result) {
				if (err) 
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_party_off', tokens, state, device_data, function(err, result) {
				if (err) 
					return console.error(err);
				}
			);
...
Homey
	.manager('flow')
	.on('condition.virtual_party', function(callback, args) {
...
Homey
	.manager('flow')
	.on('action.virtual_party_action_on', function(callback, args) {
		var virtualparty = getparty(args.device.id);
		if (virtualparty instanceof Error) 
			return callback(virtualparty);
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_party_on', tokens, state, args.device, function(err, result) {
				if (err) 
					return console.error(err);
				}
			);
...
Homey
	.manager('flow')
	.on('action.virtual_party_action_off', function(callback, args) {
		var virtualparty = getparty(args.device.id);
		if (virtualparty instanceof Error) 
			return callback(virtualparty);
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_party_off', tokens, state, args.device, function(err, result) {
				if (err) 
					return console.error(err);
				}
			);
