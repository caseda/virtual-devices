...
module.exports.capabilities.onoff.get = function(device_data, callback) {
	...
	if (virtuallight instanceof Error) 
		return callback(virtuallight);
...
module.exports.capabilities.onoff.set = function(device_data, onoff, callback) {
	...
	if (onoff) {
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_light_on', tokens, state, device_data, function(err, result) {
				if (err) 
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_light_off', tokens, state, device_data, function(err, result) {
				if (err) 
					return console.error(err);
				}
			);
...
Homey
	.manager('flow')
	.on('condition.virtual_light', function(callback, args) {
...
Homey
	.manager('flow')
	.on('action.virtual_light_action_on', function(callback, args) {
		var virtuallight = getlight(args.device.id);
		if (virtuallight instanceof Error) 
			return callback(virtuallight);
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_light_on', tokens, state, args.device, function(err, result) {
				if (err) 
					return console.error(err);
				}
			);
...
Homey
	.manager('flow')
	.on('action.virtual_light_action_off', function(callback, args) {
		var virtuallight = getlight(args.device.id);
		if (virtuallight instanceof Error) 
			return callback(virtuallight);
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_light_off', tokens, state, args.device, function(err, result) {
				if (err) 
					return console.error(err);
				}
			);
