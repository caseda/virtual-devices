...
module.exports.capabilities.onoff.get = function(device_data, callback) {
	...
	if (virtualsleep instanceof Error) 
		return callback(virtualsleep);
...
module.exports.capabilities.onoff.set = function(device_data, onoff, callback) {
	...
	if (onoff) {
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_sleep_on', tokens, state, device_data, function(err, result) {
				if (err) 
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_sleep_off', tokens, state, device_data, function(err, result) {
				if (err) 
					return console.error(err);
				}
			);
...
Homey
	.manager('flow')
	.on('condition.virtual_sleep', function(callback, args) {
...
Homey
	.manager('flow')
	.on('action.virtual_sleep_action_on', function(callback, args) {
		var virtualsleep = getsleep(args.device.id);
		if (virtualsleep instanceof Error) 
			return callback(virtualsleep);
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_sleep_on', tokens, state, args.device, function(err, result) {
				if (err) 
					return console.error(err);
				}
			);
...
Homey
	.manager('flow')
	.on('action.virtual_sleep_action_off', function(callback, args) {
		var virtualsleep = getsleep(args.device.id);
		if (virtualsleep instanceof Error) 
			return callback(virtualsleep);
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_sleep_off', tokens, state, args.device, function(err, result) {
				if (err) 
					return console.error(err);
				}
			);
