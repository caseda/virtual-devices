...
module.exports.capabilities.onoff.get = function(device_data, callback) {
	...
	if (virtualevent instanceof Error) 
		return callback(virtualevent);
...
module.exports.capabilities.onoff.set = function(device_data, onoff, callback) {
	...
	if (onoff) {
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_event_on', tokens, state, device_data, function(err, result) {
				if (err) 
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_event_off', tokens, state, device_data, function(err, result) {
				if (err) 
					return console.error(err);
				}
			);
...
Homey
	.manager('flow')
	.on('condition.virtual_event', function(callback, args) {
...
Homey
	.manager('flow')
	.on('action.virtual_event_action_on', function(callback, args) {
		var virtualevent = getevent(args.device.id);
		if (virtualevent instanceof Error) 
			return callback(virtualevent);
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_event_on', tokens, state, args.device, function(err, result) {
				if (err) 
					return console.error(err);
				}
			);
...
Homey
	.manager('flow')
	.on('action.virtual_event_action_off', function(callback, args) {
		var virtualevent = getevent(args.device.id);
		if (virtualevent instanceof Error) 
			return callback(virtualevent);
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_event_off', tokens, state, args.device, function(err, result) {
				if (err) 
					return console.error(err);
				}
			);
