...
module.exports.capabilities.button.set = function(args, button, callback) {
	...
	if (virtualButton instanceof Error) 
		return callback(virtualButton);
	...
	Homey
		.manager('flow')
		.triggerDevice('button_light_pushed', tokens, true, args, function(err, result) {
			if (err) 
...
Homey
	.manager('flow')
	.on('action.button_light_pushed', function(callback, args) {
		var virtualButton = getSwitch(args.id);
		if (virtualButton instanceof Error) 
			return callback(virtualPushButton);
