...
module.exports.capabilities.onoff.get = function(device_data, callback) {
	...
	if (modeDevice instanceof Error) 
		return callback(modeDevice);
...
module.exports.capabilities.onoff.set = function(device_data, onoff, callback) {
	...
	if (onoff) {
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_manual_on', tokens, state, device_data, function(err, result) {
				if (err) 
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_manual_off', tokens, state, device_data, function(err, result) {
				if (err) 
					return console.error(err);
				}
			);
...
Homey
	.manager('flow')
	.on('condition.virtual_manual', function(callback, args) {
...
Homey
	.manager('flow')
	.on('action.virtual_manual_action_on', function(callback, args) {
		var modeDevice = getModeDevice(args.device.id);
		if (modeDevice instanceof Error) 
			return callback(modeDevice);
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_manual_on', tokens, state, args.device, function(err, result) {
				if (err) 
					return console.error(err);
				}
			);
...
Homey
	.manager('flow')
	.on('action.virtual_manual_action_off', function(callback, args) {
		var modeDevice = getModeDevice(args.device.id);
		if (modeDevice instanceof Error) 
			return callback(modeDevice);
