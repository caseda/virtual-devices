...
module.exports.capabilities.onoff.get = function(device_data, callback) {
	...
	if (virtualalarm instanceof Error) 
		return callback(virtualalarm);
...
module.exports.capabilities.onoff.set = function(device_data, onoff, callback) {
	...
	if (onoff) {
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_alarm_on', tokens, state, device_data, function(err, result) {
				if (err) 
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_alarm_off', tokens, state, device_data, function(err, result) {
				if (err) 
					return console.error(err);
				}
			);
...
Homey
	.manager('flow')
	.on('condition.virtual_alarm', function(callback, args) {
...
Homey
	.manager('flow')
	.on('action.virtual_alarm_action_on', function(callback, args) {
		var virtualalarm = getalarm(args.device.id);
		if (virtualalarm instanceof Error) 
			return callback(virtualalarm);
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_alarm_on', tokens, state, args.device, function(err, result) {
				if (err) 
					return console.error(err);
				}
			);
...
Homey
	.manager('flow')
	.on('action.virtual_alarm_action_off', function(callback, args) {
		var virtualalarm = getalarm(args.device.id);
		if (virtualalarm instanceof Error) 
			return callback(virtualalarm);
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_alarm_off', tokens, state, args.device, function(err, result) {
				if (err) 
					return console.error(err);
				}
			);
