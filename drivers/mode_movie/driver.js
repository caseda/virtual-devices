...
module.exports.capabilities.onoff.get = function(device_data, callback) {
	...
	if (virtualmovie instanceof Error) 
		return callback(virtualmovie);
...
module.exports.capabilities.onoff.set = function(device_data, onoff, callback) {
	...
	if (onoff) {
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_movie_on', tokens, state, device_data, function(err, result) {
				if (err) 
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_movie_off', tokens, state, device_data, function(err, result) {
				if (err) 
					return console.error(err);
				}
			);
...
Homey
	.manager('flow')
	.on('condition.virtual_movie', function(callback, args) {
...
Homey
	.manager('flow')
	.on('action.virtual_movie_action_on', function(callback, args) {
		var virtualmovie = getmovie(args.device.id);
		if (virtualmovie instanceof Error) 
			return callback(virtualmovie);
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_movie_on', tokens, state, args.device, function(err, result) {
				if (err) 
					return console.error(err);
				}
			);
...
Homey
	.manager('flow')
	.on('action.virtual_movie_action_off', function(callback, args) {
		var virtualmovie = getmovie(args.device.id);
		if (virtualmovie instanceof Error) 
			return callback(virtualmovie);
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_movie_off', tokens, state, args.device, function(err, result) {
				if (err) 
					return console.error(err);
				}
			);
