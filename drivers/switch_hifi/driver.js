...
module.exports.capabilities.onoff.get = function(device_data, callback) {
	...
	if (virtualhifi instanceof Error) 
		return callback(virtualhifi);
...
module.exports.capabilities.onoff.set = function(device_data, onoff, callback) {
	...
	if (onoff) {
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_hifi_on', tokens, state, device_data, function(err, result) {
				if (err) 
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_hifi_off', tokens, state, device_data, function(err, result) {
				if (err) 
					return console.error(err);
				}
			);
...
Homey
	.manager('flow')
	.on('condition.virtual_hifi', function(callback, args) {
...
Homey
	.manager('flow')
	.on('action.virtual_hifi_action_on', function(callback, args) {
		var virtualhifi = gethifi(args.device.id);
		if (virtualhifi instanceof Error) 
			return callback(virtualhifi);
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_hifi_on', tokens, state, args.device, function(err, result) {
				if (err) 
					return console.error(err);
				}
			);
...
Homey
	.manager('flow')
	.on('action.virtual_hifi_action_off', function(callback, args) {
		var virtualhifi = gethifi(args.device.id);
		if (virtualhifi instanceof Error) 
			return callback(virtualhifi);
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_hifi_off', tokens, state, args.device, function(err, result) {
				if (err) 
					return console.error(err);
				}
			);
