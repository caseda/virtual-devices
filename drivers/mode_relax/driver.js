...
module.exports.capabilities.onoff.set = function(device_data, onoff, callback) {
	...
	if (onoff) {
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_relax_on', tokens, state, device_data, function(err, result) {
				if (err) 
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_relax_off', tokens, state, device_data, function(err, result) {
				if (err) 
					return console.error(err);
				}
			);
...
Homey
	.manager('flow')
	.on('condition.virtual_relax', function(callback, args) {
...
Homey
	.manager('flow')
	.on('action.virtual_relax_action_on', function(callback, args) {
		var modeDevice = getModeDevice(args.device.id);
		if (modeDevice instanceof Error) 
			return callback(modeDevice);
		...
		Homey
			.manager('flow')
			.triggerDevice('virtual_relax_on', tokens, state, args.device, function(err, result) {
				if (err) 
					return console.error(err);
				}
			);
...
Homey
	.manager('flow')
	.on('action.virtual_relax_action_off', function(callback, args) {
		var modeDevice = getModeDevice(args.device.id);
		if (modeDevice instanceof Error) 
			return callback(modeDevice);
